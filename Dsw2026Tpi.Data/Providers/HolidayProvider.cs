    ...
    public HolidayProvider()
    {
        ...
        if (!File.Exists(filePath))
        {
            throw new FileNotFoundException(
                "No se encontró el archivo de feriados",
                filePath);
        }

        var json = File.ReadAllText(filePath);

        var entries = JsonSerializer.Deserialize<List<HolidayEntry>>(
            json,
        ...
        foreach (var entry in entries)
        {
                    ...
                    out var date))
            {
                throw new InvalidOperationException(
                    $"La fecha '{entry.Date}' del archivo holidays.json es inválida. Debe utilizar el formato yyyy-MM-dd");
            }

            if (!_nonWorkingDays.Add(date))
            {
                throw new InvalidOperationException(
                    $"La fecha '{entry.Date}' está repetida en el archivo holidays.json.");
            }
        }
    }
